...
try:                         
    p = subprocess.Popen(r"//bhcsdbv03/emanio/Bremote%s.bat",shell=True,stdout=subprocess.PIPE,stderr=subprocess.PIPE)
    stdout, stderr = p.communicate()
    print stderr
    print 'it worked'
    with open('//bhcsdbv03/emanio/bhcsdbv03.log','a+') as testit:
        testit.write(stderr)
except:
...
logging.debug('Start chain  at '+ str(datetime.now()))   
try:                         
    p = subprocess.Popen(r"//bhcsdbv02/emanio/Bremote%s.bat",shell=True,stdout=subprocess.PIPE,stderr=subprocess.PIPE)
    stdout, stderr = p.communicate()
    print stderr
    print 'it worked'
    with open('//bhcsdbv02/emanio/bhcsdbv02.log','a+') as testit:
        testit.write(stderr)
except:
    logging.exception('Got exception on main handler chain')
    raise
