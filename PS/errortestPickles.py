    ...
    try:
        for FilePath in Files:
            ...
            try:
                ...
                SavedNewname=check_saved_new_name(DBname)
                if SavedNewname==1:
                    send_result2(DBname,'Failure',Start,Finish,0,'x',cmd,cmd2,cmd3)
                    break
                if SavedNewname==0:
                    send_result2(DBname,'Success',Start,Finish,1,'x')
            except Exception,e:   
                Finish = datetime.datetime.now().replace( microsecond=0)
                errorLevel, errorMsg = get_spss_error(e)
        ...
        send_result2('Can not open File %s' % DBname,Start,Finish)
    if SavedNewname==1:
